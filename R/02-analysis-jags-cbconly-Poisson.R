...
sink("/scratch/brolek/northwest-wrs/JAGSmodel-cbconly.txt")
...
for (s in 1:nstrata){
  for( t3 in 1:(ntime2-1) ) {
    ...
    for(j in 1:nroutes2) {  #### priors for first year and strata ######
     ...
     # derived parameters
     ##################### 
     sig.ratio[1] <- sig.obs[1]/exp(sig.proc.strat)
     sig.ratio[2] <- sig.obs[2]/exp(sig.proc.strat)
...
out <- jags(model.file="/scratch/brolek/northwest-wrs/JAGSmodel-cbconly.txt", 
